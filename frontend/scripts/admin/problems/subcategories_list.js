...
const CrudFormURL = BASE_DIR + 'frontend/admin/problems/subcategories/crud_form.php'; // Used to change the URL with the corresponding queryString (Record Id)
const BaseAPIEndpoint = BASE_DIR + 'api/v1/index.php/problems-subcategories';
...
// Table Elements -------------------------------------------------------------
const table = document.getElementById('tblProblemsSubCategories');
...
function loadStatusesOnList () {
    ...
    .then (APIResponse => {
        ...
        APIResponse.body.data.forEach(element => {
            ...
            optionItem.value = element.ProblemSubCategoryStatusId;
            optionItem.innerHTML = element.ProblemSubCategoryStatusValue;
...
function loadElementsOnTable () {
    ...
    let JSONFilters = {
        "ProblemSubCategoryStatusId": sel_statuses_filter.value,
    ...
    .then (APIResponse => {
        ...
        APIResponse.body.data.forEach(element => {
            ...
            tableRowCell.innerHTML = '<a href="'+ CrudFormURL + '?ProblemSubCategoryId=' + element.ProblemSubCategoryId + '">' + element.ProblemSubCategoryId + '</a>';
            // ProblemCategoryName
            tableRowCell = tableRow.insertCell(-1);
            tableRowCell.innerText = element.ProblemCategoryName;
            // ProblemSubCategoryName
            tableRowCell = tableRow.insertCell(-1);
            tableRowCell.innerText = element.ProblemSubCategoryName;
            // ProblemSubCategoryStatusId
            tableRowCell = tableRow.insertCell(-1);
            const ProblemSubCategoryStatusId = Number(element.ProblemSubCategoryStatusId);
