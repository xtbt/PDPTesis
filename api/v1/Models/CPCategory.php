<?php
    ...
    class CPCategory extends AppModelCore {
        
        // Class properties
        public $CPCategoryId;
        public $CPCategoryName;
        public $CPCategoryStatusId;

        // Search criteria fields string
        private $SearchCriteriaFieldsString = 'CONCAT("[",COALESCE(CPCategoryId,""),"]",COALESCE(CPCategoryName,""))';
        ...
        private function DB_initProperties() {
            $this->SQL_Tables = 'tblCPCategories';
            $this->SQL_Conditions = 'TRUE';
            $this->SQL_Order = 'CPCategoryId';
            $this->SQL_Limit = NULL;
            $this->SQL_Params = [];
        ...
        // ********************************************************************
        public function getCPCategory( $CPCategoryId ) {
            $this->DB_initProperties();
            if (is_numeric($CPCategoryId)) {
                $this->SQL_Conditions .= ' AND CPCategoryId = :CPCategoryId';
                $this->SQL_Limit = '0,1';
            }
            else {
            ...
            try {
                ...
                $this->response['data'][$CPCategoryId] = $this->SQL_Sentence->fetch(PDO::FETCH_ASSOC);
                $this->updateProperties($this->response['data'][$CPCategoryId]);
        ...
        // ********************************************************************
        public function createCPCategory( $CPCategoryName ) {
            $this->DB_initProperties();
            $CPCategoryId = NULL; // NULL by default on new records
            $CPCategoryStatusId = 1; // 1(Active) by default on new records
            try {
                $SQL_Query = 'INSERT INTO tblCPCategories VALUES (
                  :CPCategoryId, 
                  :CPCategoryName, 
                  :CPCategoryStatusId)';
                ...
                $this->SQL_Sentence = $this->DB_Connector->prepare($SQL_Query);
                $this->SQL_Sentence->bindParam(':CPCategoryId', $CPCategoryId, PDO::PARAM_INT);
                $this->SQL_Sentence->bindParam(':CPCategoryName', $CPCategoryName, PDO::PARAM_STR);
                $this->SQL_Sentence->bindParam(':CPCategoryStatusId', $CPCategoryStatusId, PDO::PARAM_INT);
                $this->SQL_Sentence->execute();
                
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $CPCategoryId = $this->DB_Connector->lastInsertId(); // Get newly created record ID
                    $this->response['count'] = 1;
                    $this->response['data'] = ['id' => $CPCategoryId];
        ...
        // ********************************************************************
        public function updateCPCategory( $CPCategoryId, $CPCategoryName ) {
            $this->getCPCategory($CPCategoryId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information

            // Confirm changes on at least 1 field ----------------------------
            if ($this->CPCategoryName == $CPCategoryName) {
            ...
            try {
                $SQL_Query = 'UPDATE tblCPCategories SET 
                  CPCategoryName = :CPCategoryName 
                  WHERE 
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getCPCategory($CPCategoryId); // Update current object data with modified info
        ...
        // ********************************************************************
        public function reactivateCPCategory($CPCategoryId) {
            $this->getCPCategory($CPCategoryId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information
            $CPCategoryStatusId = 1; // Default active status (1)

            try {
                $SQL_Query = 'UPDATE tblCPCategories SET 
                    CPCategoryStatusId = :CPCategoryStatusId 
                    WHERE 
                    CPCategoryId = :CPCategoryId';
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getCPCategory($CPCategoryId); // Update current object data after reactivation
        ...
        // ********************************************************************
        public function deactivateCPCategory($CPCategoryId) {
            $this->getCPCategory($CPCategoryId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information
            $CPCategoryStatusId = 0; // Default inactive status (0)
            ...
            try {
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getCPCategory($CPCategoryId); // Update current object data after deactivation
...
// ****************************************************************************
        ...
        public function getStatuses ($queryString = NULL) {
            ...
            try {
                ...
                $this->response['data'] = [
                    array(
                        'CPCategoryStatusId' => 0,
                        'CPCategoryStatusValue' => 'Inactive'
                    ),
                    array(
                        'CPCategoryStatusId' => 1,
                        'CPCategoryStatusValue' => 'Active'
                    )
