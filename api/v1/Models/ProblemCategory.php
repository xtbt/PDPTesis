<?php
    ...
    class ProblemCategory extends AppModelCore {
        
        // Class properties
        public $ProblemCategoryId;
        public $ProblemCategoryName;
        public $ProblemCategoryStatusId;

        // Search criteria fields string
        private $SearchCriteriaFieldsString = 'CONCAT("[",COALESCE(ProblemCategoryId,""),"]",COALESCE(ProblemCategoryName,""))';
        ...
        private function DB_initProperties() {
            $this->SQL_Tables = 'tblProblemsCategories';
            $this->SQL_Conditions = 'TRUE';
            $this->SQL_Order = 'ProblemCategoryId';
            $this->SQL_Limit = NULL;
            $this->SQL_Params = [];
        ...
        // ********************************************************************
        public function getProblemCategory($ProblemCategoryId) {
            $this->DB_initProperties();
            if (is_numeric($ProblemCategoryId)) {
                $this->SQL_Conditions .= ' AND ProblemCategoryId = :ProblemCategoryId';
                $this->SQL_Limit = '0,1';
            }
            else {
            ...
            try {
                ...
                $this->response['data'][$ProblemCategoryId] = $this->SQL_Sentence->fetch(PDO::FETCH_ASSOC);
                $this->updateProperties($this->response['data'][$ProblemCategoryId]);
        ...
        // ********************************************************************
        public function createProblemCategory($ProblemCategoryName) {
            $this->DB_initProperties();
            $ProblemCategoryId = NULL; // NULL by default on new records
            $ProblemCategoryStatusId = 1; // 1(Active) by default on new records
            try {
                $SQL_Query = 'INSERT INTO tblProblemsCategories VALUES (
                  :ProblemCategoryId, 
                  :ProblemCategoryName, 
                  :ProblemCategoryStatusId)';
                ...
                $this->SQL_Sentence = $this->DB_Connector->prepare($SQL_Query);
                $this->SQL_Sentence->bindParam(':ProblemCategoryId', $ProblemCategoryId, PDO::PARAM_INT);
                $this->SQL_Sentence->bindParam(':ProblemCategoryName', $ProblemCategoryName, PDO::PARAM_STR);
                $this->SQL_Sentence->bindParam(':ProblemCategoryStatusId', $ProblemCategoryStatusId, PDO::PARAM_INT);
                $this->SQL_Sentence->execute();
                
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $ProblemCategoryId = $this->DB_Connector->lastInsertId(); // Get newly created record ID
                    $this->response['count'] = 1;
                    $this->response['data'] = ['id' => $ProblemCategoryId];
        ...
        // ********************************************************************
        public function updateProblemCategory($ProblemCategoryId, $ProblemCategoryName) {
            $this->getProblemCategory($ProblemCategoryId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information

            // Confirm changes on at least 1 field ----------------------------
            if ($this->ProblemCategoryName == $ProblemCategoryName) {
            ...
            try {
                $SQL_Query = 'UPDATE tblProblemsCategories SET 
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getProblemCategory($ProblemCategoryId); // Update current object data with modified info
        ...
        // ********************************************************************
        public function reactivateProblemCategory($ProblemCategoryId) {
            $this->getProblemCategory($ProblemCategoryId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information
            $ProblemCategoryStatusId = 1; // Default active status (1)

            try {
                $SQL_Query = 'UPDATE tblProblemsCategories SET 
                    ProblemCategoryStatusId = :ProblemCategoryStatusId 
                    WHERE 
                    ProblemCategoryId = :ProblemCategoryId';
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getProblemCategory($ProblemCategoryId); // Update current object data after reactivation
        ...
        // ********************************************************************
        public function deactivateProblemCategory($ProblemCategoryId) {
            $this->getProblemCategory($ProblemCategoryId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information
            $ProblemCategoryStatusId = 0; // Default inactive status (0)
            ...
            try {
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getProblemCategory($ProblemCategoryId); // Update current object data after deactivation
...
// ****************************************************************************
        ...
        public function getStatuses ($queryString = NULL) {
            ...
            try {
                ...
                $this->response['data'] = [
                    array(
                        'ProblemCategoryStatusId' => 0,
                        'ProblemCategoryStatusValue' => 'Inactive'
                    ),
                    array(
                        'ProblemCategoryStatusId' => 1,
                        'ProblemCategoryStatusValue' => 'Active'
                    )
