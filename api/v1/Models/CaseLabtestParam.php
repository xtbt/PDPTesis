<?php
    ...
    class CaseLabtestParam extends AppModelCore {
        
        // Class properties
        public $CaseLabtestParamId;
        public $CaseLabtestId;
        public $CaseLabtestParam;
        public $CaseLabtestParamResult;
        public $CaseLabtestParamStatusId;

        // Search criteria fields string
        private $SearchCriteriaFieldsString = 'CONCAT("[",COALESCE(CaseLabtestParamId,""),"]",COALESCE(CaseLabtestId,""))';
        ...
        private function DB_initProperties() {
            $this->SQL_Tables = 'tblCasesLabtestsParams AS t1 LEFT JOIN 
                                tblCasesLabtests AS t2 USING(CaseLabtestId)';
            $this->SQL_Conditions = 'TRUE';
            $this->SQL_Order = 'CaseLabtestParamId';
            $this->SQL_Limit = NULL;
            $this->SQL_Params = [];
        ...
        public function getAll( $queryString = NULL ) {
            ...
            try {
                $SQL_GlobalQuery = 'SELECT 
                    t1.CaseLabtestParamId AS CaseLabtestParamId, 
                    t1.CaseLabtestId AS CaseLabtestId, 
                    t1.CaseLabtestParam AS CaseLabtestParam, 
                    t1.CaseLabtestParamResult AS CaseLabtestParamResult, 
                    t1.CaseLabtestParamStatusId AS CaseLabtestParamStatusId 
                    FROM '
                    .$this->SQL_Tables.
                    ' WHERE '
                    .$this->SQL_Conditions.
        ...
        // ********************************************************************
        public function getCaseLabtestParam( $CaseLabtestParamId ) {
            $this->DB_initProperties();
            if (is_numeric($CaseLabtestParamId)) {
                $this->SQL_Conditions .= ' AND CaseLabtestParamId = :CaseLabtestParamId';
                $this->SQL_Limit = '0,1';
            }
            else {
            ...
            try {
                ...
                $this->SQL_Sentence = $this->DB_Connector->prepare($SQL_Query);
                $this->SQL_Sentence->bindParam(':CaseLabtestParamId', $CaseLabtestParamId, PDO::PARAM_INT);
                $this->SQL_Sentence->execute();
                if ($this->SQL_Sentence->rowCount() < 1) {
                    $this->response['count'] = 0; // No records found
                    $this->response['globalCount'] = 0; // No records found
                ...
                $this->response['data'][$CaseLabtestParamId] = $this->SQL_Sentence->fetch(PDO::FETCH_ASSOC);
                $this->updateProperties($this->response['data'][$CaseLabtestParamId]);
        ...
        // ********************************************************************
        public function createCaseLabtestParam( $CaseLabtestId, $CaseLabtestParam, $CaseLabtestParamResult ) {
            $this->DB_initProperties();
            $CaseLabtestParamId = NULL; // NULL by default on new records
            $CaseLabtestParam = new DateTime(date('Y-m-d H:i:s')); // Current DateTime object
            $CaseLabtestParam = $CaseLabtestParam->format('Y-m-d'); // String converted
            $CaseLabtestParamStatusId = 1; // 1(Pending) by default on new records
            try {
                $SQL_Query = 'INSERT INTO tblCasesLabtestsParams VALUES (
                    :CaseLabtestParamId, 
                    :CaseLabtestId, 
                    :CaseLabtestParam, 
                    :CaseLabtestParamResult, 
                    :CaseLabtestParamStatusId)';
                  
                $this->SQL_Sentence = $this->DB_Connector->prepare($SQL_Query);
                $this->SQL_Sentence->bindParam(':CaseLabtestParamId', $CaseLabtestParamId, PDO::PARAM_INT);
                $this->SQL_Sentence->bindParam(':CaseLabtestId', $CaseLabtestId, PDO::PARAM_INT);
                $this->SQL_Sentence->bindParam(':CaseLabtestParam', $CaseLabtestParam, PDO::PARAM_STR);
                $this->SQL_Sentence->bindParam(':CaseLabtestParamResult', $CaseLabtestParamResult, PDO::PARAM_STR);
                $this->SQL_Sentence->bindParam(':CaseLabtestParamStatusId', $CaseLabtestParamStatusId, PDO::PARAM_INT);
                $this->SQL_Sentence->execute();
                
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $CaseLabtestParamId = $this->DB_Connector->lastInsertId(); // Get newly created record ID
                    $this->response['count'] = 1;
                    $this->response['data'] = ['id' => $CaseLabtestParamId];
        ...
        // ********************************************************************
        public function updateCaseLabtestParam( $CaseLabtestParamId, $CaseLabtestId, $CaseLabtestParam, $CaseLabtestParamResult ) {
            $this->getCaseLabtestParam($CaseLabtestParamId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information

            // Confirm changes on at least 1 field ----------------------------
            if ( $this->CaseLabtestId == $CaseLabtestId && $this->CaseLabtestParam == $CaseLabtestParam 
            && $this->CaseLabtestParamResult == $CaseLabtestParamResult ) {
            ...
            try {
                $SQL_Query = 'UPDATE tblCasesLabtestsParams SET 
                    CaseLabtestId = :CaseLabtestId, 
                    CaseLabtestParam = :CaseLabtestParam, 
                    CaseLabtestParamResult = :CaseLabtestParamResult 
                    WHERE 
                    CaseLabtestParamId = :CaseLabtestParamId';
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getCaseLabtestParam($CaseLabtestParamId); // Update current object data with modified info
        ...
        // ********************************************************************
        public function reactivateCaseLabtestParam( $CaseLabtestParamId ) {
            $this->getCaseLabtestParam($CaseLabtestParamId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information
            $CaseLabtestParamStatusId = 1; // Default active status (1)

            try {
                $SQL_Query = 'UPDATE tblCasesLabtestsParams SET 
                    CaseLabtestParamStatusId = :CaseLabtestParamStatusId 
                    WHERE 
                    CaseLabtestParamId = :CaseLabtestParamId';
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getCaseLabtestParam($CaseLabtestParamId); // Update current object data after reactivation
        ...
        // ********************************************************************
        public function deactivateCaseLabtestParam( $CaseLabtestParamId ) {
            $this->getCaseLabtestParam($CaseLabtestParamId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information
            $CaseLabtestParamStatusId = 0; // Default inactive status (0)
            ...
            try {
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getCaseLabtestParam($CaseLabtestParamId); // Update current object data after deactivation
...
// ****************************************************************************
        public function getStatuses ( $queryString = NULL ) {
            ...
            try {
                ...
                $this->response['data'] = [
                    array(
                        'CaseLabtestParamStatusId' => 0,
                        'CaseLabtestParamStatusValue' => 'Inactive'
                    ),
                    array(
                        'CaseLabtestParamStatusId' => 1,
                        'CaseLabtestParamStatusValue' => 'Active'
                    )
