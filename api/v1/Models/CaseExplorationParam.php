<?php
    ...
    class CaseExplorationParam extends AppModelCore {
        
        // Class properties
        public $CaseExplorationParamId;
        public $CaseExplorationId;
        public $CaseExplorationParam;
        public $CaseExplorationParamResult;
        public $CaseExplorationParamStatusId;

        // Search criteria fields string
        private $SearchCriteriaFieldsString = 'CONCAT("[",COALESCE(CaseExplorationParamId,""),"]",COALESCE(CaseExplorationId,""))';
        ...
        private function DB_initProperties() {
            $this->SQL_Tables = 'tblCasesExplorationsParams AS t1 LEFT JOIN 
                                tblCasesExplorations AS t2 USING(CaseExplorationId)';
            $this->SQL_Conditions = 'TRUE';
            $this->SQL_Order = 'CaseExplorationParamId';
            $this->SQL_Limit = NULL;
            $this->SQL_Params = [];
        ...
        public function getAll( $queryString = NULL ) {
            ...
            try {
                $SQL_GlobalQuery = 'SELECT 
                    t1.CaseExplorationParamId AS CaseExplorationParamId, 
                    t1.CaseExplorationId AS CaseExplorationId, 
                    t1.CaseExplorationParam AS CaseExplorationParam, 
                    t1.CaseExplorationParamResult AS CaseExplorationParamResult, 
                    t1.CaseExplorationParamStatusId AS CaseExplorationParamStatusId 
                    FROM '
                    .$this->SQL_Tables.
                    ' WHERE '
                    .$this->SQL_Conditions.
        ...
        // ********************************************************************
        public function getCaseExplorationParam( $CaseExplorationParamId ) {
            $this->DB_initProperties();
            if (is_numeric($CaseExplorationParamId)) {
                $this->SQL_Conditions .= ' AND CaseExplorationParamId = :CaseExplorationParamId';
                $this->SQL_Limit = '0,1';
            }
            else {
            ...
            try {
                ...
                $this->SQL_Sentence = $this->DB_Connector->prepare($SQL_Query);
                $this->SQL_Sentence->bindParam(':CaseExplorationParamId', $CaseExplorationParamId, PDO::PARAM_INT);
                $this->SQL_Sentence->execute();
                if ($this->SQL_Sentence->rowCount() < 1) {
                    $this->response['count'] = 0; // No records found
                    $this->response['globalCount'] = 0; // No records found
                ...
                $this->response['data'][$CaseExplorationParamId] = $this->SQL_Sentence->fetch(PDO::FETCH_ASSOC);
                $this->updateProperties($this->response['data'][$CaseExplorationParamId]);
        ...
        // ********************************************************************
        public function createCaseExplorationParam( $CaseExplorationId, $CaseExplorationParam, $CaseExplorationParamResult ) {
            $this->DB_initProperties();
            $CaseExplorationParamId = NULL; // NULL by default on new records
            $CaseExplorationParam = new DateTime(date('Y-m-d H:i:s')); // Current DateTime object
            $CaseExplorationParam = $CaseExplorationParam->format('Y-m-d'); // String converted
            $CaseExplorationParamStatusId = 1; // 1(Pending) by default on new records
            try {
                $SQL_Query = 'INSERT INTO tblCasesExplorationsParams VALUES (
                    :CaseExplorationParamId, 
                    :CaseExplorationId, 
                    :CaseExplorationParam, 
                    :CaseExplorationParamResult, 
                    :CaseExplorationParamStatusId)';
                  
                $this->SQL_Sentence = $this->DB_Connector->prepare($SQL_Query);
                $this->SQL_Sentence->bindParam(':CaseExplorationParamId', $CaseExplorationParamId, PDO::PARAM_INT);
                $this->SQL_Sentence->bindParam(':CaseExplorationId', $CaseExplorationId, PDO::PARAM_INT);
                $this->SQL_Sentence->bindParam(':CaseExplorationParam', $CaseExplorationParam, PDO::PARAM_STR);
                $this->SQL_Sentence->bindParam(':CaseExplorationParamResult', $CaseExplorationParamResult, PDO::PARAM_STR);
                $this->SQL_Sentence->bindParam(':CaseExplorationParamStatusId', $CaseExplorationParamStatusId, PDO::PARAM_INT);
                $this->SQL_Sentence->execute();
                
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $CaseExplorationParamId = $this->DB_Connector->lastInsertId(); // Get newly created record ID
                    $this->response['count'] = 1;
                    $this->response['data'] = ['id' => $CaseExplorationParamId];
        ...
        // ********************************************************************
        public function updateCaseExplorationParam( $CaseExplorationParamId, $CaseExplorationId, $CaseExplorationParam, $CaseExplorationParamResult ) {
            $this->getCaseExplorationParam($CaseExplorationParamId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information

            // Confirm changes on at least 1 field ----------------------------
            if ( $this->CaseExplorationId == $CaseExplorationId && $this->CaseExplorationParam == $CaseExplorationParam 
            && $this->CaseExplorationParamResult == $CaseExplorationParamResult ) {
            ...
            try {
                $SQL_Query = 'UPDATE tblCasesExplorationsParams SET 
                    CaseExplorationId = :CaseExplorationId, 
                    CaseExplorationParam = :CaseExplorationParam, 
                    CaseExplorationParamResult = :CaseExplorationParamResult 
                    WHERE 
                    CaseExplorationParamId = :CaseExplorationParamId';
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getCaseExplorationParam($CaseExplorationParamId); // Update current object data with modified info
        ...
        // ********************************************************************
        public function reactivateCaseExplorationParam( $CaseExplorationParamId ) {
            $this->getCaseExplorationParam($CaseExplorationParamId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information
            $CaseExplorationParamStatusId = 1; // Default active status (1)

            try {
                $SQL_Query = 'UPDATE tblCasesExplorationsParams SET 
                    CaseExplorationParamStatusId = :CaseExplorationParamStatusId 
                    WHERE 
                    CaseExplorationParamId = :CaseExplorationParamId';
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getCaseExplorationParam($CaseExplorationParamId); // Update current object data after reactivation
        ...
        // ********************************************************************
        public function deactivateCaseExplorationParam( $CaseExplorationParamId ) {
            $this->getCaseExplorationParam($CaseExplorationParamId); // Get current record data from DB
            $this->initResponseData(); // Reset Response Array Information
            $CaseExplorationParamStatusId = 0; // Default inactive status (0)
            ...
            try {
                ...
                if ($this->SQL_Sentence->rowCount() != 0) {
                    $this->getCaseExplorationParam($CaseExplorationParamId); // Update current object data after deactivation
...
// ****************************************************************************
        public function getStatuses ( $queryString = NULL ) {
            ...
            try {
                ...
                $this->response['data'] = [
                    array(
                        'CaseExplorationParamStatusId' => 0,
                        'CaseExplorationParamStatusValue' => 'Inactive'
                    ),
                    array(
                        'CaseExplorationParamStatusId' => 1,
                        'CaseExplorationParamStatusValue' => 'Active'
                    )
